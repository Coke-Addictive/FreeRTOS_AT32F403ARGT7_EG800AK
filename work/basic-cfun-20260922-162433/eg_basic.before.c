...
static SemaphoreHandle_t    s_basic_at_completion_sem = NULL;       // Basic 后台任务等待自己的 AT 请求完成
 ...
 * @param completion_sem 当前调用任务专用的二值信号量。
 * @return Service 返回的 AT 执行结果
 * @note Basic 和 Service 必须初始化成功；仅供任务上下文串行调用，禁止在 URC 回调中调用。同一信号量同一时间只允许对应一条未完成请求
 */
static Eg800AtResult_e Eg800_Basic_At_Execute(Eg800BasicCmd_e cmd_id, const char *name, const char *tx, const char *rsp_prefix, char *rx_buf, size_t rx_buf_size, Eg800AtResultInfo_t *result_info, SemaphoreHandle_t completion_sem) {
...
static void Eg800_Basic_Task(void *parameters) {
    ...
    while (1) {
        ...
        (void)Eg800_Basic_Query_Sim(&info.sim_state, s_basic_at_completion_sem);
        (void)Eg800_Basic_Query_Signal(&info.csq, &info.ber, s_basic_at_completion_sem);
        (void)Eg800_Basic_Query_Imei(info.imei, sizeof(info.imei), s_basic_at_completion_sem);
        (void)Eg800_Basic_Query_Imsi(info.imsi, sizeof(info.imsi), s_basic_at_completion_sem);
        (void)Eg800_Basic_Query_Iccid(info.iccid, sizeof(info.iccid), s_basic_at_completion_sem);
        (void)Eg800_Basic_Query_Phone_Number(info.phone_number, sizeof(info.phone_number), s_basic_at_completion_sem);
        (void)Eg800_Basic_Query_Firmware_Version(info.firmware_version, sizeof(info.firmware_version), s_basic_at_completion_sem);
        (void)Eg800_Basic_Query_Operator(&info.operator_type, info.operator_name, sizeof(info.operator_name), s_basic_at_completion_sem);
...
Eg800AtResult_e Eg800_Basic_Set_Echo_Close(SemaphoreHandle_t completion_sem) {
    return Eg800_Basic_At_Execute(EG800_BASIC_CMD_ATE0, "关闭AT回显", "ATE0\r\n", NULL, NULL, 0U, NULL, completion_sem);
}

// 将模块 URC 上报端口设置为 UART1
Eg800AtResult_e Eg800_Basic_Set_Urc_Port_Uart1(SemaphoreHandle_t completion_sem) {
    return Eg800_Basic_At_Execute(EG800_BASIC_CMD_URC_PORT, "设置 URC 上报口为 UART1", "AT+QURCCFG=\"urcport\",\"uart1\"\r\n", NULL, NULL, 0U, NULL, completion_sem);
}

// 将 RI 信号类型设置为物理引脚输出。
Eg800AtResult_e Eg800_Basic_Set_Ri_Physical(SemaphoreHandle_t completion_sem) {
 ...
 */
Eg800AtResult_e Eg800_Basic_Query_Signal(uint8_t *csq, uint8_t *ber, SemaphoreHandle_t completion_sem) {
 ...
 */
Eg800AtResult_e Eg800_Basic_Query_Sim(Eg800BasicSimState_e *sim_state, SemaphoreHandle_t completion_sem) {
    char response[EG800_BASIC_AT_RESPONSE_MAX_SIZE];
 ...
 */
Eg800AtResult_e Eg800_Basic_Query_Imei(char *imei, size_t imei_size, SemaphoreHandle_t completion_sem) {
    char response[EG800_BASIC_AT_RESPONSE_MAX_SIZE];
 ...
 * @return AT命令执行结果；响应为空或空间不足时返回DATA_FAIL。
 */
Eg800AtResult_e Eg800_Basic_Query_Imsi(char *imsi, size_t imsi_size, SemaphoreHandle_t completion_sem) {
    char response[EG800_BASIC_AT_RESPONSE_MAX_SIZE];
 ...
 */
Eg800AtResult_e Eg800_Basic_Query_Iccid(char *iccid, size_t iccid_size, SemaphoreHandle_t completion_sem) {
    char response[EG800_BASIC_AT_RESPONSE_MAX_SIZE];
 ...
 */
Eg800AtResult_e Eg800_Basic_Query_Phone_Number(char *phone_number, size_t phone_number_size, SemaphoreHandle_t completion_sem) {
    char response[EG800_BASIC_AT_RESPONSE_MAX_SIZE];
 ...
 */
Eg800AtResult_e Eg800_Basic_Query_Firmware_Version(char *version, size_t version_size, SemaphoreHandle_t completion_sem) {
    char response[EG800_BASIC_AT_RESPONSE_MAX_SIZE];
 ...
 * @return AT命令执行结果；响应格式错误或空间不足时返回DATA_FAIL。
 */
Eg800AtResult_e Eg800_Basic_Query_Operator(Eg800BasicOperator_e *operator_type, char *operator_name, size_t operator_name_size, SemaphoreHandle_t completion_sem) {
    char response[EG800_BASIC_AT_RESPONSE_MAX_SIZE];
