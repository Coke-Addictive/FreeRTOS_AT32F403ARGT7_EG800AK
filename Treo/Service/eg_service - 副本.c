...
typedef struct {
    const char *prefix;                                                     /* URC 前缀 */
    uint8_t owner;                                                          /* URC 归属模块 */
    Eg800AtUrcHandler_t handler;                                            /* URC 处理函数 */
} Eg800UrcRoute_t;
...
static Eg800UrcRoute_t s_eg800_urc_routes[EG800_SERVICE_URC_ROUTE_MAX];     // URC路由表存储区
...
static void Eg800_Queue_Item_Complete(Eg800AtQueueItem_t *item, Eg800AtResult_e result, int16_t module_error) {
    ...
    // 通知提交该AT请求的任务
    if (item->notify_task != NULL) {
        (void)xTaskNotifyGiveIndexed(item->notify_task,EG800_AT_COMPLETION_NOTIFY_INDEX);
    }
}
...
/**
 * @brief 将响应行追加到当前请求的RX_BUF文本响应缓冲区。
 * @param line 需要保存的响应行。
 */
static void Eg800_Request_Append_Rx(const char *line) {
    Eg800AtRequest_t *request;
...
static void Eg800_Handle_Line(const char *line) {
    ...
    /* WAIT_RAW数据不经过文本行处理 */
    LOG("当前状态未处理文本: state=%u, line=%s", (unsigned int)s_eg800_at_state, line);
}
...
static bool Eg800_Route_Urc(const char *line) {
    ...
    LOG("收到 URC路由: owner= %u, line= %s", (unsigned int)owner, line);
    handler(line);
    return true;
}
 ...
 */
static void Eg800_Service_Wait_Event(void) {
    (void)ulTaskNotifyTakeIndexed(EG800_SERVICE_NOTIFY_INDEX, pdTRUE, pdMS_TO_TICKS(EG800_SERVICE_EVENT_WAIT_MS));
}
...
Eg800AtResult_e Eg800_AtExec(const Eg800AtRequest_t *request, Eg800AtResultInfo_t *result_info) {
    Eg800AtQueueItem_t item;
    Eg800AtResultInfo_t local_result_info;
    Eg800AtResultInfo_t *out_info;
    ...
    /* 服务未初始化或者请求参数不合法 */
    if ((s_eg800_at_queue == NULL) || (s_eg800_service_task_handle == NULL) || (Eg800_Request_Validate(request) == false)) {
    ...
    //清除通知槽位 1 中可能残留的旧完成通知，防止把上一次通知误认为本次请求已经完成
    (void)ulTaskNotifyTakeIndexed(EG800_AT_COMPLETION_NOTIFY_INDEX, pdTRUE, 0U);
    ...
    (void)xTaskNotifyGiveIndexed(s_eg800_service_task_handle, EG800_SERVICE_NOTIFY_INDEX);
    notify_value = ulTaskNotifyTakeIndexed(EG800_AT_COMPLETION_NOTIFY_INDEX, pdTRUE, pdMS_TO_TICKS(EG800_AT_SYNC_MAX_WAIT_MS));
    if (notify_value == 0U) {
        // 这 185 秒包括：请求在队列中等待，+ AT 命令执行，+ 等待响应/URC/数据。走到这里通常说明服务任务卡住状态机异常或请求没有被正确完成
...
void Eg800_Service_Rx_Notify_From_ISR(void) {
    ...
    vTaskNotifyGiveIndexedFromISR(s_eg800_service_task_handle, EG800_SERVICE_NOTIFY_INDEX, &higher_priority_task_woken);
